...
class MyMqtt {
   private:
    ...
    void connect() {
        ...
        while (!_client.connected()) {
            ...
            // Attempt to connect
            if (_client.connect(_clientId.c_str(), MY_MQTT_USERNAME,
                                MY_MQTT_PASSWORD, _lwtTopic.c_str(), QOS,
                                RETAIN, "offline")) {
                ...
                _client.publish(_deviceNameTopic.c_str(), _deviceName.c_str(),
                                RETAIN);
   ...
   public:
    ...
    void begin() {
        ...
        _client.setCallback([this](const char* topic, const byte* payload,
                                   unsigned int length) {
...
//     //==============MESSAGE RECEPTION HERE (Uncomment to
//     //edit)====================
//     if (messageTemp == mqtt_Race_Setting) {
//         messageMQTT = incommingMessage;
//         newMessage = true;
//     }
// }
