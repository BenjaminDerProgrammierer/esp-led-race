...
MyDisplay display = MyDisplay("Open LED Race", "V5");
MyWifi wifi = MyWifi();
...
void setup() {
  ...
  wifi.connect();
  display.showStatus("Connected to WiFi",
                     ("SSID: " + wifi.getWifiSSID()).c_str(),
                     ("IP: " + wifi.getWifiIP()).c_str());
  delay(2000);
  display.showStatus("Connecting to MQTT");
  mqtt.begin();
  mqtt.publishWifiData(wifi.getWifiSSID(), wifi.getWifiIP(), wifi.getWifiMAC());
  display.showStatus("Connected to MQTT",
                     ("SSID: " + wifi.getWifiSSID()).c_str(),
                     ("IP: " + wifi.getWifiIP()).c_str());
  delay(2000);
  game.begin();
